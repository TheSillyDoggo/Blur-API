...
void setupPostProcess()
{
    ...
    auto vertexPath = Blur::getShaderPath(true);
    auto fragmentPath = Blur::getShaderPath(false);
    ...
    ppShaderPass = glGetUniformLocation(ppShader.program, "pass");
}
...
void Blur::drawBlurredNode(CCNode* node, BlurAPI::BlurOptions* options)
{
    int PASSES = BlurAPIOptions::passes;
    float blur = 1.0f;
    ...
    for (int i = 0; i < PASSES; ++i)
    {
        ...
        glBindTexture(
            GL_TEXTURE_2D,
            ping ? ppRt0.tex : ppRt1.tex
        );

        glUniform1i(ppShaderPass, i + 1);
